...
async def signup_for_access_token(
    ...
    # 이메일 중복 확인
    user_info = await crud.get_user_info_by_email(db=db, email=user_info_data.email)
    if user_info:
    ...
    # 비밀번호 해싱
    hashed_password = security.get_password_hash(password=user_info_data.password)
    user_info_data.password = hashed_password

    # 새로운 사용자 정보 추가
    new_user_info = await crud.create_user_info(db=db, user_info_data=user_info_data)

    # JWT 토큰 발급
    access_token = security.create_access_token(data={"sub": new_user_info.email})
...
async def login_for_access_token(
    ...
    if not user_info:
        response.handle_bad_request(detail="Incorrect email.", code="INVALID_EMAIL")

    # 비밀번호 확인
    if not security.verify_password(
        plain_password=login_data.password, hashed_password=user_info.password
    ):
        response.handle_bad_request(detail="Incorrect password.", code="INVALID_PWD")
